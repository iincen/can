...
module.exports = async(xinz, msg, _afk, welcome, left) => {
    try {
        ...
        const fakeTroli = {key: {fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? {remoteJid: "status@broadcast" } : {}) }, message: {orderMessage: {itemCount: 2021, status: 200, surface: 200, message: "<𝑨𝒓𝒅𝒚ッ>", orderTitle: "</𝑨𝒓𝒅𝒚ッ>", thumbnail: fs.readFileSync('./media/thumbnail.png'), sellerJid: '0@s.whatsapp.net'}}}
...
textImg(`🌹 *Prefix Active :* 🌹

≻ *Multiprefix* : ${xinz.multi ? '𝒀𝒆𝒔' : '𝑵𝒐'}
≻ *Noprefix* : ${xinz.nopref ? '𝒀𝒆𝒔' : '𝑵𝒐'}
≻ *Singleprefix* : ${xinz.nopref ? '𝑵𝒐' : '𝒀𝒆𝒔'}`)
}
break
...
let menuall = `${ucapanWaktu} ✨✨
Ram : _${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}MB / ${Math.round(require('os').totalmem / 1024 / 1024)}MB_
Battery : _${xinz.baterai.baterai}% ${xinz.baterai.cas === 'true' ? 'Sedang di Charger ✅⚡' : 'Tidak di Charger ❌🔌'}_
- - - - - - - - - - - - -
...
_Silahkan tunggu file media sedang dikirim mungkin butuh beberapa menit_`
            ...
            case prefix+'play':
                ...
                sendButloc(from, ``, `Video Or Audio ?\n© By @Ardy`, await getBuffer('https://img.youtube.com/vi/' + res.videos[0].videoId + '/sddefault.jpg'), qqppp, {contextInfo:{mentionedJid: [sender]}})
                    limitAdd(sender, limit)
                })
                    .catch((err) => reply(`${err}`))
                } catch (err) {
...
const buttons = [
  ...
  {buttonId: `${prefix}suit kertas`, buttonText: {displayText: 'Kertas 📜'}, type: 1},
              ...
              if (!q) return sendButtons(from, 'Silahkan pilih list suit di bawah', 'Suit Message By Ardy🥴', buttons, {quoted: msg})
              const userspilih = q
              if (!userspilih.match(/batu|gunting|kertas/)) return sendButtons(from, 'Silahkan pilih list suit di bawah', 'Suit Message By Ardy🥴', buttons, {quoted: msg})
...
.then((res) => sendButtons(from, 'Partner di temukan dengan nama , silahkan chat dia!', botName, [{buttonId: `${command}`, buttonText: {displayText: 'Search Again'}, type: "RESPONSE"},{buttonId: `${prefix}menu`, buttonText: {displayText: 'Back ⬅️'}, type: "RESPONSE"}], {quoted: res}))
